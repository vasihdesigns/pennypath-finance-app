...
## Control Center / Lock Screen quick-add (iOS 18)

The easiest way to log an expense is the iOS 18 **Control**, implemented in the
`PennyPathWidgets` extension target:

- [`PennyPathWidgets/PennyPathWidgetBundle.swift`](PennyPathWidgets/PennyPathWidgetBundle.swift) —
  the `AddExpenseControl`. Its button runs `AddExpenseIntent`, which is shared
  from the app ([`QuickAddIntent.swift`](PennyPath/App/QuickAddIntent.swift) is a
  member of both targets). Because the intent sets `openAppWhenRun = true`,
  tapping the control opens PennyPath straight to the Add Expense sheet.
- The extension is iOS 18+ (Controls require 18); the app itself stays iOS 17.
- It's embedded in the app via the "Embed Foundation Extensions" build phase and
  builds as part of the normal `PennyPath` scheme — no extra steps to build.

**Signing note:** the widget has its own bundle id
`com.vasih.PennyPath.PennyPathWidgets`. With automatic signing and a paid
developer account, Xcode provisions it for you on the first device build /
archive — no entitlements or capabilities are needed for this plain control.

**To verify it (device only):** Controls don't show on the Simulator. On a real
iPhone, long-press Control Center → **＋ Add a Control** → find **PennyPath →
Add Expense**, or add it to the Lock Screen. Tapping it should open the app to
the expense form.
