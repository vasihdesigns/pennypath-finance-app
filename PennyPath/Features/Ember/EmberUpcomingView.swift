//
//  EmberUpcomingView.swift
//  PennyPath
//
//  Ember's "Upcoming" screen — subscriptions and scheduled future payments.
...
struct EmberUpcomingView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingStats) {
            EmberSubsStatsSheet(subscriptions: subscriptions, currencyCode: currencyCode)
        }
    }
        ...
        VStack(spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                ...
                stat("DUE SOON", value: "\(dueSoonCount)",
                     ...
                     valueColor: dueSoonCount > 0 ? Ember.spend : Ember.onCanvas)
            }
            ...
            HStack(alignment: .center, spacing: 12) {
                ...
                Toggle("", isOn: $includeCommitments)
                    .labelsHidden()
                    .tint(Ember.accentSoft)
            }
        }
        .emberPanel(padding: 18)
    }

    private func stat(_ overline: String, value: String, caption: String,
                      valueColor: Color = Ember.onCanvas) -> some View {
    ...
    private var statDivider: some View {
        Rectangle().fill(Ember.onCanvasSoft.opacity(0.18)).frame(width: 1, height: 46)
    }
    ...
    private func pill(_ title: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            Text(title)
                ...
                .lineLimit(1)
                .foregroundStyle(active ? Ember.plusInk : Ember.onCanvas)
                .padding(.vertical, 9)
                .padding(.horizontal, 13)
                .background {
                    if active { Capsule().fill(Ember.plus) }
                    else { Capsule().fill(Ember.onCanvasSoft.opacity(0.14)) }
                }
        }
        .buttonStyle(.plain)
    }
    ...
    private func iconButton(_ symbol: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            Image(systemName: symbol)
                ...
                .foregroundStyle(active ? Ember.accentSoft : Ember.onCanvasSoft)
    ...
    private var searchField: some View {
        ...
        .overlay(Capsule().strokeBorder(Ember.glassStroke(dark: scheme == .dark), lineWidth: 1))
    }
            ...
            HStack(spacing: 12) {
                if !payment.iconURL.isEmpty, let url = URL(string: payment.iconURL) {
                    ...
                    Text(String(payment.name.prefix(1)).uppercased())
                        ...
                        .background(Ember.accentSoft.opacity(0.16),
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(dueLabel(payment))
                        .font(.system(size: 12))
                        .foregroundStyle(payment.isOverdue ? Ember.spend : Ember.onCanvasSoft)
                }
                Spacer(minLength: 8)
...
private struct EmberSubsStatsSheet: View {
                            ...
                            HStack(spacing: 12) {
                                VStack(alignment: .leading, spacing: 6) {
                                    Text(item.category?.title ?? "Uncategorized")
                                        ...
                                        .foregroundStyle(Ember.onCanvas)
                                    EmberMeter(value: item.monthly / maxValue, tint: Ember.accentSoft, height: 8)
                                }
