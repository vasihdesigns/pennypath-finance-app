//
//  MonoNetWorthView.swift
//  PennyPath
//
//  Mono's Net Worth screen on real data: one honest number (everything you own
//  minus everything you owe), then each money kind as a charcoal card lapping into
...
struct MonoNetWorthView: View {
    ...
    @State private var expandedKind: MonoMoneyKind?
    ...
    private func accountsIn(_ kind: MonoMoneyKind) -> [Account] {
        accounts.filter { !$0.isMarketLinked && MonoMoneyKind.of($0.category) == kind }
    }
    private func total(for kind: MonoMoneyKind) -> Double {
        var sum = accountsIn(kind).reduce(0) { $0 + $1.signedBalance }
        if kind == .investment { sum += investmentsValue }
        return sum
    }
    private var activeKinds: [MonoMoneyKind] {
        MonoMoneyKind.allCases.filter { kind in
    ...
    private var allocationSlices: [MonoAllocationSlice] {
        ...
        return activeKinds.compactMap { kind in
            ...
            return MonoAllocationSlice(kind: kind, value: value, pct: value / gross * 100)
        }
        .sorted { $0.value > $1.value }
    }

    private func subtitle(for kind: MonoMoneyKind) -> String {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                MonoHeader(title: "Net Worth (\(currencyCode))",
                ...
                Text(money(displayedNetWorth, code: currencyCode))
                    ...
                    .foregroundStyle(displayedNetWorth >= 0 ? Mono.onCanvas : Mono.spend)
                    .lineLimit(1)
                    .minimumScaleFactor(0.5)
                    .padding(.top, 8)

                HStack(spacing: 14) {
                    MonoGlassPill(label: "Assets", value: money(assetTotal, code: currencyCode))
                    MonoGlassPill(label: "Liabilities", value: money(liabilityTotal, code: currencyCode))
                }
                .padding(.top, 18)
        ...
        .sheet(isPresented: $showingAdd) { MonoAddAccountView() }
        ...
        .fullScreenCover(isPresented: $showingAllocation) {
            MonoAllocationView(slices: allocationSlices, netWorth: netWorth,
    ...
    private func deckCard(_ kind: MonoMoneyKind, isLast: Bool) -> some View {
        ...
        let shape = RoundedRectangle(cornerRadius: Mono.cardRadius, style: .continuous)
    ...
    private func cardHeader(_ kind: MonoMoneyKind, value: Double, isExpanded: Bool) -> some View {
    ...
    private func subRow(name: String, value: String, kind: MonoMoneyKind,
    ...
    private func addRow(_ title: String, kind: MonoMoneyKind, cardIsDark: Bool,
