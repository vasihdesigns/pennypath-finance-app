//
//  MonoBudgetView.swift
//  PennyPath
//
//  Mono's Budget screen — the monthly plan you compare against real spending.
...
//  the real BudgetFormView; bars run grey while under, near-black when over.
//

import SwiftUI
import SwiftData

struct MonoBudgetView: View {
    ...
    private var headline: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(money(abs(left), code: currencyCode))
                ...
                .foregroundStyle(over ? Mono.spend : Mono.good)
                .lineLimit(1)
                .minimumScaleFactor(0.5)
            MonoMeter(value: totalBudget > 0 ? min(1, totalSpent / totalBudget) : 0,
                       tint: over ? Mono.spend : Mono.good, height: 12)
    ...
    private func categoryRow(_ budget: CategoryBudget) -> some View {
        ...
        let tint = isOver ? Mono.spend : Mono.good
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            MonoMeter(value: progress, tint: tint, height: 10)
    ...
    private var emptyCard: some View {
        VStack(spacing: 14) {
            ...
            Text("Make a spending plan")
                ...
                .foregroundStyle(Mono.onCanvas)
            Text("Set a monthly budget for the things you spend on. Each bar fills as you spend, and flags when you go over.")
                .font(.system(size: 14))
                .foregroundStyle(Mono.onCanvasSoft)
    ...
    private func softButton(title: String, systemImage: String?, action: @escaping () -> Void) -> some View {
        Button {
            ...
            .frame(maxWidth: .infinity)
            .padding(.vertical, 13)
            .background(Mono.accentSoft.opacity(0.16), in: Capsule())
        }
        .buttonStyle(.plain)
    }
