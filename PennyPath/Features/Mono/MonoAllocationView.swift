...
//  assets. No tab bar, no scrolling. Grayscale ramp, deepest box for the biggest
//  share.
//
...
/// total assets.
struct MonoAllocationSlice: Identifiable {
    let kind: MonoMoneyKind
    let value: Double
    let pct: Double
    var id: String { kind.id }
}

struct MonoAllocationView: View {
    @Environment(\.colorScheme) private var scheme
    /// Asset slices, sorted by value descending, each with value > 0.
    let slices: [MonoAllocationSlice]
    let netWorth: Double
    ...
    /// reads as the same deck — lighter charcoal for Cash down to black for Liability.
    private func tile(_ slice: MonoAllocationSlice, size: CGSize) -> some View {
        let kind = slice.kind
