//
//  SpectrumUpcomingView.swift
//  PennyPath
//
//  Spectrum's "Upcoming" screen — subscriptions and scheduled future payments.
//  A three-stat header (monthly subs · total upcoming · due soon), then the list
//  split into Subscriptions (recurring) and Future Payments (one-off), with
//  search and a quick stats breakdown.
//
...
struct SpectrumUpcomingView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingAdd) {
            // Subscriptions sub-tab → opens repeating; Future Payments sub-tab →
            // opens as a one-off scheduled for tomorrow. Both land in Upcoming.
            SpectrumAddItemView(initialRepeats: subTab == .subscriptions,
                                initialDate: subTab == .future ? Date.now.adding(days: 1) : .now)
        }
        .sheet(item: $editing) { SpectrumAddItemView(payment: $0) }
        .sheet(isPresented: $showingStats) {
            SpectrumSubsStatsSheet(subscriptions: subscriptions, currencyCode: currencyCode)
        }
    }
    ...
    private var statsCard: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            stat("DUE SOON", value: "\(dueSoonCount)",
                 ...
                 valueColor: dueSoonCount > 0 ? Spectrum.spend : Spectrum.onCanvas)
        }
        .spectrumPanel(padding: 18)
    }

    private func stat(_ overline: String, value: String, caption: String,
                      valueColor: Color = Spectrum.onCanvas) -> some View {
    ...
    private var statDivider: some View {
        Rectangle().fill(Spectrum.onCanvasSoft.opacity(0.18)).frame(width: 1, height: 46)
    }
    ...
    private func pill(_ title: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            Text(title)
                ...
                .lineLimit(1)
                .foregroundStyle(active ? Spectrum.plusInk : Spectrum.onCanvas)
                .padding(.vertical, 9)
                .padding(.horizontal, 13)
                .background {
                    if active { Capsule().fill(Spectrum.plus) }
                    else { Capsule().fill(Spectrum.onCanvasSoft.opacity(0.14)) }
                }
        }
        .buttonStyle(.plain)
    }
    ...
    private func iconButton(_ symbol: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button {
            ...
            Image(systemName: symbol)
                ...
                .foregroundStyle(active ? Spectrum.accentSoft : Spectrum.onCanvasSoft)
    ...
    private var searchField: some View {
        ...
        .overlay(Capsule().strokeBorder(Spectrum.glassStroke(dark: scheme == .dark), lineWidth: 1))
    }
            ...
            HStack(spacing: 12) {
                if !payment.iconURL.isEmpty, let url = URL(string: payment.iconURL) {
                    ...
                    Text(String(payment.name.prefix(1)).uppercased())
                        ...
                        .background(Spectrum.accentSoft.opacity(0.16),
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(dueLabel(payment))
                        .font(.system(size: 12))
                        .foregroundStyle(payment.isOverdue ? Spectrum.spend : Spectrum.onCanvasSoft)
                }
                Spacer(minLength: 8)
...
private struct SpectrumSubsStatsSheet: View {
                            ...
                            HStack(spacing: 12) {
                                VStack(alignment: .leading, spacing: 6) {
                                    Text(item.category?.title ?? "Uncategorized")
                                        ...
                                        .foregroundStyle(Spectrum.onCanvas)
                                    SpectrumMeter(value: item.monthly / maxValue, tint: Spectrum.accentSoft, height: 8)
                                }
