...
//  assets. No tab bar, no scrolling. One jewel tone per kind, biggest box for the biggest
//  share.
//
...
/// total assets.
struct SpectrumAllocationSlice: Identifiable {
    let kind: SpectrumMoneyKind
    let value: Double
    let pct: Double
    var id: String { kind.id }
}

struct SpectrumAllocationView: View {
    @Environment(\.colorScheme) private var scheme
    /// Asset slices, sorted by value descending, each with value > 0.
    let slices: [SpectrumAllocationSlice]
    let netWorth: Double
    ...
    /// reads as the same deck — each kind in its own jewel tone.
    private func tile(_ slice: SpectrumAllocationSlice, size: CGSize) -> some View {
        let kind = slice.kind
